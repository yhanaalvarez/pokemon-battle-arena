...
export class PostgresDAO implements DAO {
  async init() {
    ...
    const adminResult = await pool.query(`
      ...
      FROM pmba.user
      WHERE username = 'DSTRYR'
    `);

    if (adminResult.rows[0].count == 0) {
      logDAO("Creating admin account");

      const adminUser = new User({
        username: "DSTRYR",
        hashed_password: "125715",
        salt: "125715",
  ...
  async findUser(username, includeSecrets = false) {
  ...
  async saveChallenge(challenge) {
    ...
    await pool.query(
      ...
      UPDATE SET json=$2
    `,
      [challenge.challengeId, challengeJson]
    );
  }
